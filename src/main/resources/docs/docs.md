...
The `home-routes` handler in the `org.atomicsauce.skein.routes.home` namespace
...
The `home-page` function will in turn call the `org.atomicsauce.skein.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `org.atomicsauce.skein.handler` namespace:
...
Request middleware functions are located under the `skein.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `org.atomicsauce.skein.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `org.atomicsauce.skein.db.core` in some other namespace.
